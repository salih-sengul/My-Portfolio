...
const svgComponentsModules = import.meta.glob("../assets/imgs/*.svg", {
  query: "?react",
  eager: true,
});
...
export default function DynamicSvg({ imageName, ...props }) {
  // props like fill, className, width, height
  const modulePath = `../assets/imgs/${imageName}`;
  const SvgComponent =
    svgComponentsModules[modulePath]?.default ||
    svgComponentsModules[modulePath] ||
    FallbackSvgComponent;
